...
if __name__ == '__main__':
    ...
    while True:
        ...
        for idx, elem in enumerate(population):
            ...
            if elem[1] is None: # Only calculate value if not previously calculated
                ...
                for season in range(2006, 2016): 
                    print '\tPREMIER LEAGUE ' + str(season) + '-' + str(season+1) + ': ',
                    money_left, matches = runner.play(INITIAL_MONEY, ESTIMATOR, BETTING_STRATEGY, league=u'Premier League', season=str(season) + '-' + str(season+1))
                    print money_left
        ...
        with open('populations_premier_league/' + time.strftime('%Y-%m-%d_%H:%M:%s') + '.pickle', 'wb') as f:
